...
def get_word_dict(utterances):  # Need to remove stop words and test
    stop_words = list()
    ...
    for utterance in utterances:
        for u in utterance:
            u = re.sub("\[\d*\]", "", u)  # replace [1], [2], etc as ''
            u = re.sub(r'[~`!#$%^&*()_+-=|\';":/.,?><~·！@#￥%……&*（）——+-=“：’；、。，？》《{}]+', "", u)
...
def get_test_data(word_dict, goal_type_dict, goal_entity_dict):
    ...
    with open('../origin_data/test.txt', 'r') as f:
        for  idx, line in enumerate(f.readlines()):
            ...
            if utterance == "":
                continue
            utterance = re.sub("\[\d*\]", "", utterance)  # replace [1], [2], etc as ''
...
def text_generator(word_dict, documents):
    ...
    for doc in documents:
        ...
        for line in doc:
            ...
            doc_idx.append(line_idx)  # 2d list
        texts_idx.append(doc_idx)  # 3d list
    return texts_idx


def get_data_idx(utt, type, entity, bot, word_dict, type_dict, entity_dict, bot_dict):
    utt_idx = text_generator(word_dict, utt)  # 3d list, texts_idx -> doc_idx -> word_idx
    type_idx = list()
    entity_idx = list()
    bot_idx = list()
...
if __name__ == '__main__':
    ...
    print("word_dict size: %d" % (len(word_dict)))
    ...
    all_goal_entity = set.union(all_goal_entity, all_goal_type)
