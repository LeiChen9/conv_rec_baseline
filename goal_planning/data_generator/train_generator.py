...
class Dataset(object):
        ...
        for idx in range(len(self.utterance)):
            ...
            for jdx in range(line_len):
                if self.bot[idx][jdx] == 1:
                    ...
                    binary_label.append(self.label[idx][jdx])
        return binary_utterance, binary_goal_type, binary_label
    ...
    def next_goal_data(self, undersample=False):
        ...
        return binary_goal_type, binary_goal_type_label, binary_goal_type_idx, \
                binary_goal_entity, binary_goal_entity_label, binary_goal_entity_idx, \
                binary_final_goal_type, binary_final_goal_entity
...
def get_data(data_tag, undersample=False):
    ...
    print("Sample NUmber: %d, Jump Number: %d, Jump Rate: %.2f" % (
        len(binary_utterance), np.sum(binary_label), float(np.sum(binary_label))/len(binary_utterance)
    ))
    print("Next Goal Type Prediction...")
    print("Sample Number: %d, True Number: %d, True Rate: %.2f" % (
        len(next_goal_type), np.sum(next_goal_type_label), float(np.sum(next_goal_entity_label))/len(next_goal_type)
    ))
